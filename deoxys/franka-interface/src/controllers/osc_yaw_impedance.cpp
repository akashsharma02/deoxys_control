...
OSCYawImpedanceController::OSCYawImpedanceController() {}
OSCYawImpedanceController::~OSCYawImpedanceController() {}

OSCYawImpedanceController::OSCYawImpedanceController(franka::Model &model) {
  model_ = &model;
}

bool OSCYawImpedanceController::ParseMessage(const FrankaControlMessage &msg) {
...
void OSCYawImpedanceController::ComputeGoal(
  ...
  if (control_msg_.goal().is_delta()) {
    ...
    // Need test
    Eigen::AngleAxisd current_axis_angle(
        current_state_info->quat_EE_in_base_frame * default_quat.inverse());
    double current_z =
        current_axis_angle.angle() * current_axis_angle.axis()[2];

    Eigen::Vector3d current_ori(0., 0., current_z);
    AxisAngle(current_ori, current_axis_angle);

    Eigen::AngleAxisd relative_axis_angle;
    double az;

    // (Yifeng) Current hacked to to clip the value of yaw angle
    if ((-1.6 > current_z && control_msg_.goal().az() < 0.) ||
        (current_z > 2.7 && control_msg_.goal().az() > 0.)) {
      az = 0.;
    } else {
      az = control_msg_.goal().az();
    }
    Eigen::Vector3d relative_ori(0, 0, az);
    AxisAngle(relative_ori, relative_axis_angle);

    goal_state_info->quat_EE_in_base_frame = Eigen::Quaterniond(
        relative_axis_angle.toRotationMatrix() *
        current_axis_angle.toRotationMatrix() * default_quat);
    ...
    goal_state_info->pos_EE_in_base_frame = Eigen::Vector3d(
        control_msg_.goal().x(), control_msg_.goal().y(),
        control_msg_.goal().z());
...
std::array<double, 7> OSCYawImpedanceController::Step(
  ...
  // w, x, y, z
  Eigen::Quaterniond fixed_desired_quat_EE_in_base_frame(1., 0., 0., 0.);
  fixed_desired_quat_EE_in_base_frame =
      desired_quat_EE_in_base_frame * fixed_desired_quat_EE_in_base_frame;
